...
import Data.Effect.Class.TH (
    MakeEmptyEffectData (NoMakeEmptyEffectData),
...
makeEffectData defaultEffectDataNamer NoMakeEmptyEffectData ''Provider

instance HFunctor (ProviderH c i g) where
    hfmap phi (Provide i f) = Provide i \l -> f $ l . phi

instance Send (ProviderH c i g) f => Provider c i g (SendVia f) where
    {-# INLINE provide #-}
